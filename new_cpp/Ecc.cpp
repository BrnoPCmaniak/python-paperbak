...
void Encode8(unsigned char *data,unsigned char *bb,int pad) {
        int i,j;
...
int Decode8(unsigned char *data,int *eras_pos,int no_eras,int pad) {
        ...
        unsigned char lambda[33],s[32],b[33],t[33],omega[33];
        unsigned char root[32],reg[33],loc[32];
        for (i=0; i<32; i++)
                s[i]=data[0];
        for (j=1; j<255-pad; j++) {
                for (i=0; i<32; i++) {
        ...
        if (no_eras>0) {
                ...
                for (i=1; i<no_eras; i++) {
                        u=(unsigned char)((11*(254-eras_pos[i]))%255);
                        for (j=i+1; j>0; j--) {
        ...
        while (++r<=32) {
                ...
                if (discr_r==255) {
                        ...
                        if (2*el<=r+no_eras-1) {
                                el=r+no_eras-el;
                                for (i=0; i<=32; i++)
                                        b[i]=(unsigned char)(lambda[i]==0 ? 255 : (ecc_index[lambda[i]]-discr_r+255)%255);
                                ;
                        }
                        else {
                                memmove(b+1,b,32);
        ...
        for (i=1,k=115; i<=255; i++,k=(k+116)%255) {
                ...
                for (j=deg_lambda; j>0; j--) {
                        if (reg[j]!=255) {
                                reg[j]=(unsigned char)((reg[j]+j)%255);
                                q^=alpha[reg[j]];
                        };
                };
                if (q!=0) continue;
